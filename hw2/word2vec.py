...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(outsideVectors[indices[0]].dot(centerWordVec))) - np.log(sigmoid(-outsideVectors[indices[1:]].dot(centerWordVec))).sum()

    gradCenterVec = -(1-sigmoid(outsideVectors[indices[0]].dot(centerWordVec)))*outsideVectors[indices[0]] + (1-sigmoid(-outsideVectors[indices[1:]].dot(centerWordVec))).dot(outsideVectors[indices[1:]])

    gradOutsideVecs = np.zeros(outsideVectors.shape)

    for i in range(K):
        gradOutsideVecs[indices[i+1]]+=(1-sigmoid(-outsideVectors[indices[i+1]].dot(centerWordVec)))*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsidewordInd = [word2Ind[a] for a in outsideWords]

    final_ls = [word2vecLossAndGradient(centerWordVector, outsidewordInd[k], outsideVectors, dataset) for k in range(len(outsidewordInd))]
    loss_ls,center_ls, outside_ls = zip(*final_ls)
    loss += sum(loss_ls)
    gradCenterVecs[centerWordInd] += sum(center_ls)
    gradOutsideVectors += sum(outside_ls)
    ### END YOUR CODE
